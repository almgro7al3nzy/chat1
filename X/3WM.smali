.class public abstract LX/3WM;
.super Ljava/lang/Object;
.source ""

# interfaces
.implements LX/0DJ;
.implements LX/0DK;
.implements LX/0DM;


# static fields
.field public static final A0E:[J


# instance fields
.field public A00:I

.field public A01:I

.field public A02:J

.field public A03:J

.field public A04:J

.field public A05:J

.field public A06:J

.field public A07:J

.field public A08:J

.field public A09:J

.field public A0A:J

.field public A0B:J

.field public A0C:[B

.field public A0D:[J


# direct methods
.method public static constructor <clinit>()V
    .locals 1

    const/16 v0, 0x50

    new-array v0, v0, [J

    fill-array-data v0, :array_0

    sput-object v0, LX/3WM;->A0E:[J

    return-void

    :array_0
    .array-data 8
        0x428a2f98d728ae22L    # 3.5989662528217666E12
        0x7137449123ef65cdL    # 2.367405559035152E237
        -0x4a3f043013b2c4d1L    # -9.0786554787018E-50
        -0x164a245a7e762444L
        0x3956c25bf348b538L
        0x59f111f1b605d019L    # 1.8054860536953294E125
        -0x6dc07d5b50e6b065L    # -8.717181310573585E-221
        -0x54e3a12a25927ee8L
        -0x27f855675cfcfdbeL    # -1.1656291332323638E116
        0x12835b0145706fbeL
        0x243185be4ee4b28cL    # 2.410773468256423E-134
        0x550c7dc3d5ffb4e2L    # 4.985403983718413E101
        0x72be5d74f27b896fL    # 5.183352230240388E244
        -0x7f214e01c4e9694fL
        -0x6423f958da38edcbL
        -0x3e640e8b3096d96cL    # -1.1720222785268623E8
        -0x1b64963e610eb52eL    # -4.339261227288659E176
        -0x1041b879c7b0da1dL    # -1.836300920646737E230
        0xfc19dc68b8cd5b5L    # 8.864787397362889E-233
        0x240ca1cc77ac9c65L    # 4.924067956729057E-135
        0x2de92c6f592b0275L    # 1.5818166760957606E-87
        0x4a7484aa6ea6e483L    # 4.79798724707082E50
        0x5cb0a9dcbd41fbd4L    # 3.100593885146353E138
        0x76f988da831153b5L    # 1.2864857866870779E265
        -0x67c1aead11992055L
        -0x57ce3992d24bcdf0L    # -4.511217866312199E-115
        -0x4ffcd8376704dec1L    # -2.0678662886600765E-77
        -0x40a680384110f11cL    # -0.0015563440936014411
        -0x391ff40cc257703eL    # -2.603722742751637E33
        -0x2a586eb86cf558dbL    # -4.222814448133811E104
        0x6ca6351e003826fL
        0x142929670a0e6e70L
        0x27b70a8546d22ffcL
        0x2e1b21385c26c926L    # 1.3637893953385892E-86
        0x4d2c6dfc5ac42aedL    # 5.84763610164635E63
        0x53380d139d95b3dfL    # 7.838866619197482E92
        0x650a73548baf63deL    # 5.35921865865203E178
        0x766a0abb3c77b2a8L    # 2.5625906234442426E262
        -0x7e3d36d1b812511aL    # -3.506432391784029E-300
        -0x6d8dd37aeb7dcac5L    # -8.044358981173315E-220
        -0x5d40175eb30efc9cL
        -0x57e599b443bdcfffL
        -0x3db4748f2f07686fL    # -2.366070870891841E11
        -0x3893ae5cf9ab41d0L    # -1.17632082693375E36
        -0x2e6d17e62910ade8L    # -9.182337425192181E84
        -0x2966f9dbaa9a56f0L    # -1.4692477645833556E109
        -0xbf1ca7aa88edfd6L
        0x106aa07032bbd1b8L
        0x19a4c116b8d2d0c8L
        0x1e376c085141ab53L    # 4.067301537801791E-163
        0x2748774cdf8eeb99L    # 1.894937972556452E-119
        0x34b0bcb5e19b48a8L    # 6.82593759724882E-55
        0x391c0cb3c5c95a63L    # 1.3505399862746614E-33
        0x4ed8aa4ae3418acbL    # 6.809319594147137E71
        0x5b9cca4f7763e373L    # 2.0435436325319052E133
        0x682e6ff3d6b2b8a3L    # 6.943421982965376E193
        0x748f82ee5defb2fcL    # 2.887850816088868E253
        0x78a5636f43172f60L    # 1.4463210820003646E273
        -0x7b3787eb5e0f548eL
        -0x7338fdf7e59bc614L
        -0x6f410005dc9ce1d8L    # -5.111680914909667E-228
        -0x5baf9314217d4217L    # -9.03940504491957E-134
        -0x41065c084d3986ebL    # -2.445268471406536E-5
        -0x398e870d1c8dacd5L    # -2.2148969568888243E31
        -0x35d8c13115d99e64L    # -1.6986554718624063E49
        -0x2e794738de3f3df9L    # -5.517613964471652E84
        -0x15258229321f14e2L    # -5.315750124715331E206
        -0xa82b08011912e88L    # -8.801976642581914E257
        0x6f067aa72176fbaL
        0xa637dc5a2c898a6L
        0x113f9804bef90daeL
        0x1b710b35131c471bL
        0x28db77f523047d84L    # 7.138679721560702E-112
        0x32caab7b40c72493L    # 5.064907242937011E-64
        0x3c9ebe0a15c9bebcL    # 1.0665892261952011E-16
        0x431d67c49c100d4cL    # 2.069217113539411E15
        0x4cc5d4becb3e42b6L    # 7.016224550123326E61
        0x597f299cfc657e2aL    # 1.2875119238090917E123
        0x5fcb6fab3ad6faecL    # 2.873901462601813E153
        0x6c44198c4a475817L    # 3.3832852265175575E213
    .end array-data
.end method

.method public constructor <init>()V
    .locals 1

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    const/16 v0, 0x8

    new-array v0, v0, [B

    iput-object v0, p0, LX/3WM;->A0C:[B

    const/16 v0, 0x50

    new-array v0, v0, [J

    iput-object v0, p0, LX/3WM;->A0D:[J

    const/4 v0, 0x0

    iput v0, p0, LX/3WM;->A01:I

    invoke-virtual {p0}, LX/3WM;->reset()V

    return-void
.end method

.method public constructor <init>(LX/3WM;)V
    .locals 1

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    const/16 v0, 0x8

    new-array v0, v0, [B

    iput-object v0, p0, LX/3WM;->A0C:[B

    const/16 v0, 0x50

    new-array v0, v0, [J

    iput-object v0, p0, LX/3WM;->A0D:[J

    invoke-virtual {p0, p1}, LX/3WM;->A04(LX/3WM;)V

    return-void
.end method

.method public static final A00(J)J
    .locals 6

    const/16 v0, 0x24

    shl-long v4, p0, v0

    const/16 v0, 0x1c

    ushr-long v0, p0, v0

    or-long/2addr v4, v0

    const/16 v0, 0x1e

    shl-long v2, p0, v0

    const/16 v0, 0x22

    ushr-long v0, p0, v0

    or-long/2addr v2, v0

    xor-long/2addr v4, v2

    const/16 v0, 0x19

    shl-long v1, p0, v0

    const/16 v0, 0x27

    ushr-long/2addr p0, v0

    or-long/2addr p0, v1

    xor-long/2addr p0, v4

    return-wide p0
.end method

.method public static final A01(J)J
    .locals 6

    const/16 v0, 0x32

    shl-long v4, p0, v0

    const/16 v0, 0xe

    ushr-long v0, p0, v0

    or-long/2addr v4, v0

    const/16 v0, 0x2e

    shl-long v2, p0, v0

    const/16 v0, 0x12

    ushr-long v0, p0, v0

    or-long/2addr v2, v0

    xor-long/2addr v4, v2

    const/16 v0, 0x17

    shl-long v1, p0, v0

    const/16 v0, 0x29

    ushr-long/2addr p0, v0

    or-long/2addr p0, v1

    xor-long/2addr p0, v4

    return-wide p0
.end method


# virtual methods
.method public A02()V
    .locals 57

    move-object/from16 v56, p0

    invoke-virtual/range {p0 .. p0}, LX/3WM;->A03()V

    const/16 v5, 0x10

    :goto_0
    const/16 v0, 0x4f

    if-gt v5, v0, :cond_0

    move-object/from16 v0, v56

    iget-object v4, v0, LX/3WM;->A0D:[J

    add-int/lit8 v0, v5, -0x2

    aget-wide v10, v4, v0

    const/16 v0, 0x2d

    shl-long v6, v10, v0

    const/16 v0, 0x13

    ushr-long v0, v10, v0

    or-long/2addr v6, v0

    const/4 v0, 0x3

    shl-long v2, v10, v0

    const/16 v0, 0x3d

    ushr-long v0, v10, v0

    or-long/2addr v2, v0

    xor-long/2addr v6, v2

    const/4 v0, 0x6

    ushr-long/2addr v10, v0

    xor-long/2addr v10, v6

    add-int/lit8 v0, v5, -0x7

    aget-wide v0, v4, v0

    add-long/2addr v10, v0

    add-int/lit8 v0, v5, -0xf

    aget-wide v8, v4, v0

    const/16 v0, 0x3f

    shl-long v6, v8, v0

    const/4 v0, 0x1

    ushr-long v0, v8, v0

    or-long/2addr v6, v0

    const/16 v0, 0x38

    shl-long v2, v8, v0

    const/16 v0, 0x8

    ushr-long v0, v8, v0

    or-long/2addr v2, v0

    xor-long/2addr v6, v2

    const/4 v0, 0x7

    ushr-long/2addr v8, v0

    xor-long/2addr v8, v6

    add-long/2addr v10, v8

    add-int/lit8 v0, v5, -0x10

    aget-wide v0, v4, v0

    add-long/2addr v10, v0

    aput-wide v10, v4, v5

    add-int/lit8 v5, v5, 0x1

    goto :goto_0

    :cond_0
    move-object/from16 v0, v56

    iget-wide v0, v0, LX/3WM;->A02:J

    move-wide/from16 v47, v0

    move-object/from16 v0, v56

    iget-wide v15, v0, LX/3WM;->A03:J

    iget-wide v12, v0, LX/3WM;->A04:J

    iget-wide v10, v0, LX/3WM;->A05:J

    iget-wide v8, v0, LX/3WM;->A06:J

    iget-wide v6, v0, LX/3WM;->A07:J

    move-wide/from16 v33, v10

    iget-wide v4, v0, LX/3WM;->A08:J

    move-wide/from16 v19, v47

    iget-wide v2, v0, LX/3WM;->A09:J

    move-wide/from16 v45, v2

    move-wide/from16 v29, v4

    move-wide/from16 v21, v6

    const/16 v55, 0x0

    const/4 v14, 0x0

    move-wide/from16 v27, v15

    move-wide/from16 v39, v8

    move-wide/from16 v31, v12

    :goto_1
    const/16 v0, 0xa

    move/from16 v1, v55

    if-ge v1, v0, :cond_1

    invoke-static/range {v39 .. v40}, LX/3WM;->A01(J)J

    move-result-wide v36

    and-long v17, v21, v39

    const-wide/16 v53, -0x1

    xor-long v0, v39, v53

    and-long v0, v0, v29

    xor-long v0, v0, v17

    add-long v36, v36, v0

    sget-object v52, LX/3WM;->A0E:[J

    aget-wide v0, v52, v14

    add-long v36, v36, v0

    move-object/from16 v0, v56

    iget-object v0, v0, LX/3WM;->A0D:[J

    move-object/from16 v51, v0

    add-int/lit8 v35, v14, 0x1

    aget-wide v0, v0, v14

    add-long v36, v36, v0

    add-long v36, v36, v45

    add-long v33, v33, v36

    invoke-static/range {v19 .. v20}, LX/3WM;->A00(J)J

    move-result-wide v25

    move-wide/from16 v49, v19

    and-long v23, v19, v27

    and-long v19, v19, v31

    xor-long v19, v19, v23

    and-long v0, v27, v31

    xor-long v19, v19, v0

    add-long v25, v25, v19

    add-long v25, v25, v36

    invoke-static/range {v33 .. v34}, LX/3WM;->A01(J)J

    move-result-wide v17

    move-wide/from16 v45, v33

    and-long v0, v39, v33

    xor-long v33, v33, v53

    and-long v33, v33, v21

    xor-long v33, v33, v0

    add-long v17, v17, v33

    aget-wide v0, v52, v35

    add-long v17, v17, v0

    add-int/lit8 v14, v35, 0x1

    aget-wide v0, v51, v35

    add-long v17, v17, v0

    add-long v17, v17, v29

    add-long v31, v31, v17

    invoke-static/range {v25 .. v26}, LX/3WM;->A00(J)J

    move-result-wide v35

    move-wide/from16 v43, v25

    and-long v33, v25, v49

    and-long v25, v25, v27

    xor-long v25, v25, v33

    xor-long v25, v25, v23

    add-long v35, v35, v25

    add-long v35, v35, v17

    invoke-static/range {v31 .. v32}, LX/3WM;->A01(J)J

    move-result-wide v24

    and-long v17, v45, v31

    xor-long v0, v31, v53

    and-long v0, v0, v39

    xor-long v0, v0, v17

    add-long v24, v24, v0

    aget-wide v0, v52, v14

    add-long v24, v24, v0

    add-int/lit8 v19, v14, 0x1

    aget-wide v0, v51, v14

    add-long v24, v24, v0

    add-long v24, v24, v21

    add-long v27, v27, v24

    invoke-static/range {v35 .. v36}, LX/3WM;->A00(J)J

    move-result-wide v29

    move-wide/from16 v41, v35

    and-long v22, v35, v43

    and-long v35, v35, v49

    xor-long v35, v35, v22

    xor-long v35, v35, v33

    add-long v29, v29, v35

    add-long v29, v29, v24

    invoke-static/range {v27 .. v28}, LX/3WM;->A01(J)J

    move-result-wide v17

    move-wide/from16 v24, v27

    and-long v0, v31, v27

    xor-long v27, v27, v53

    and-long v27, v27, v45

    xor-long v27, v27, v0

    add-long v17, v17, v27

    aget-wide v0, v52, v19

    add-long v17, v17, v0

    add-int/lit8 v14, v19, 0x1

    aget-wide v0, v51, v19

    add-long v17, v17, v0

    add-long v17, v17, v39

    add-long v49, v49, v17

    invoke-static/range {v29 .. v30}, LX/3WM;->A00(J)J

    move-result-wide v39

    move-wide/from16 v37, v29

    and-long v20, v29, v41

    and-long v29, v29, v43

    xor-long v29, v29, v20

    xor-long v29, v29, v22

    add-long v39, v39, v29

    add-long v39, v39, v17

    invoke-static/range {v49 .. v50}, LX/3WM;->A01(J)J

    move-result-wide v17

    move-wide/from16 v35, v49

    and-long v0, v24, v49

    xor-long v49, v49, v53

    and-long v49, v49, v31

    xor-long v49, v49, v0

    add-long v17, v17, v49

    aget-wide v0, v52, v14

    add-long v17, v17, v0

    add-int/lit8 v19, v14, 0x1

    aget-wide v0, v51, v14

    add-long v17, v17, v0

    add-long v17, v17, v45

    add-long v43, v43, v17

    invoke-static/range {v39 .. v40}, LX/3WM;->A00(J)J

    move-result-wide v33

    and-long v22, v39, v37

    and-long v0, v39, v41

    xor-long v0, v0, v22

    xor-long v0, v0, v20

    add-long v33, v33, v0

    add-long v33, v33, v17

    invoke-static/range {v43 .. v44}, LX/3WM;->A01(J)J

    move-result-wide v17

    move-wide/from16 v45, v43

    and-long v0, v35, v43

    xor-long v43, v43, v53

    and-long v43, v43, v24

    xor-long v43, v43, v0

    add-long v17, v17, v43

    aget-wide v0, v52, v19

    add-long v17, v17, v0

    add-int/lit8 v14, v19, 0x1

    aget-wide v0, v51, v19

    add-long v17, v17, v0

    add-long v17, v17, v31

    add-long v41, v41, v17

    invoke-static/range {v33 .. v34}, LX/3WM;->A00(J)J

    move-result-wide v31

    and-long v20, v33, v39

    and-long v0, v33, v37

    xor-long v0, v0, v20

    xor-long v0, v0, v22

    add-long v31, v31, v0

    add-long v31, v31, v17

    invoke-static/range {v41 .. v42}, LX/3WM;->A01(J)J

    move-result-wide v18

    move-wide/from16 v29, v41

    and-long v0, v45, v41

    xor-long v41, v41, v53

    and-long v41, v41, v35

    xor-long v41, v41, v0

    add-long v18, v18, v41

    aget-wide v0, v52, v14

    add-long v18, v18, v0

    add-int/lit8 v17, v14, 0x1

    aget-wide v0, v51, v14

    add-long v18, v18, v0

    add-long v18, v18, v24

    add-long v37, v37, v18

    invoke-static/range {v31 .. v32}, LX/3WM;->A00(J)J

    move-result-wide v27

    and-long v25, v31, v33

    and-long v0, v31, v39

    xor-long v0, v0, v25

    xor-long v0, v0, v20

    add-long v27, v27, v0

    add-long v27, v27, v18

    invoke-static/range {v37 .. v38}, LX/3WM;->A01(J)J

    move-result-wide v23

    move-wide/from16 v21, v37

    and-long v0, v29, v37

    xor-long v37, v37, v53

    and-long v37, v37, v45

    xor-long v37, v37, v0

    add-long v23, v23, v37

    aget-wide v0, v52, v17

    add-long v23, v23, v0

    add-int/lit8 v14, v17, 0x1

    aget-wide v0, v51, v17

    add-long v23, v23, v0

    add-long v23, v23, v35

    add-long v39, v39, v23

    invoke-static/range {v27 .. v28}, LX/3WM;->A00(J)J

    move-result-wide v19

    and-long v17, v27, v31

    and-long v0, v27, v33

    xor-long v0, v0, v17

    xor-long v0, v0, v25

    add-long v19, v19, v0

    add-long v19, v19, v23

    add-int/lit8 v55, v55, 0x1

    goto/16 :goto_1

    :cond_1
    add-long v47, v47, v19

    move-wide/from16 v0, v47

    move-object/from16 v14, v56

    iput-wide v0, v14, LX/3WM;->A02:J

    add-long v15, v15, v27

    iput-wide v15, v14, LX/3WM;->A03:J

    add-long v12, v12, v31

    iput-wide v12, v14, LX/3WM;->A04:J

    add-long v10, v10, v33

    iput-wide v10, v14, LX/3WM;->A05:J

    add-long v8, v8, v39

    iput-wide v8, v14, LX/3WM;->A06:J

    add-long v6, v6, v21

    iput-wide v6, v14, LX/3WM;->A07:J

    add-long v4, v4, v29

    iput-wide v4, v14, LX/3WM;->A08:J

    add-long v2, v2, v45

    iput-wide v2, v14, LX/3WM;->A09:J

    const/4 v4, 0x0

    iput v4, v14, LX/3WM;->A00:I

    const/16 v3, 0x10

    :goto_2
    if-ge v4, v3, :cond_2

    iget-object v2, v14, LX/3WM;->A0D:[J

    const-wide/16 v0, 0x0

    aput-wide v0, v2, v4

    add-int/lit8 v4, v4, 0x1

    goto :goto_2

    :cond_2
    return-void
.end method

.method public final A03()V
    .locals 8

    iget-wide v4, p0, LX/3WM;->A0A:J

    const-wide v6, 0x1fffffffffffffffL

    cmp-long v0, v4, v6

    if-lez v0, :cond_0

    iget-wide v2, p0, LX/3WM;->A0B:J

    const/16 v0, 0x3d

    ushr-long v0, v4, v0

    add-long/2addr v2, v0

    iput-wide v2, p0, LX/3WM;->A0B:J

    and-long/2addr v4, v6

    iput-wide v4, p0, LX/3WM;->A0A:J

    :cond_0
    return-void
.end method

.method public A04(LX/3WM;)V
    .locals 4

    iget-object v2, p1, LX/3WM;->A0C:[B

    iget-object v1, p0, LX/3WM;->A0C:[B

    array-length v0, v2

    const/4 v3, 0x0

    invoke-static {v2, v3, v1, v3, v0}, Ljava/lang/System;->arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V

    iget v0, p1, LX/3WM;->A01:I

    iput v0, p0, LX/3WM;->A01:I

    iget-wide v0, p1, LX/3WM;->A0A:J

    iput-wide v0, p0, LX/3WM;->A0A:J

    iget-wide v0, p1, LX/3WM;->A0B:J

    iput-wide v0, p0, LX/3WM;->A0B:J

    iget-wide v0, p1, LX/3WM;->A02:J

    iput-wide v0, p0, LX/3WM;->A02:J

    iget-wide v0, p1, LX/3WM;->A03:J

    iput-wide v0, p0, LX/3WM;->A03:J

    iget-wide v0, p1, LX/3WM;->A04:J

    iput-wide v0, p0, LX/3WM;->A04:J

    iget-wide v0, p1, LX/3WM;->A05:J

    iput-wide v0, p0, LX/3WM;->A05:J

    iget-wide v0, p1, LX/3WM;->A06:J

    iput-wide v0, p0, LX/3WM;->A06:J

    iget-wide v0, p1, LX/3WM;->A07:J

    iput-wide v0, p0, LX/3WM;->A07:J

    iget-wide v0, p1, LX/3WM;->A08:J

    iput-wide v0, p0, LX/3WM;->A08:J

    iget-wide v0, p1, LX/3WM;->A09:J

    iput-wide v0, p0, LX/3WM;->A09:J

    iget-object v2, p1, LX/3WM;->A0D:[J

    iget-object v1, p0, LX/3WM;->A0D:[J

    array-length v0, v2

    invoke-static {v2, v3, v1, v3, v0}, Ljava/lang/System;->arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V

    iget v0, p1, LX/3WM;->A00:I

    iput v0, p0, LX/3WM;->A00:I

    return-void
.end method

.method public A05([BI)V
    .locals 8

    iget-object v7, p0, LX/3WM;->A0D:[J

    iget v6, p0, LX/3WM;->A00:I

    invoke-static {p1, p2}, LX/0DO;->A0D([BI)I

    move-result v2

    add-int/lit8 v0, p2, 0x4

    invoke-static {p1, v0}, LX/0DO;->A0D([BI)I

    move-result v1

    int-to-long v4, v2

    const-wide v2, 0xffffffffL

    and-long/2addr v4, v2

    const/16 v0, 0x20

    shl-long/2addr v4, v0

    int-to-long v0, v1

    and-long/2addr v0, v2

    or-long/2addr v0, v4

    aput-wide v0, v7, v6

    add-int/lit8 v1, v6, 0x1

    iput v1, p0, LX/3WM;->A00:I

    const/16 v0, 0x10

    if-ne v1, v0, :cond_0

    invoke-virtual {p0}, LX/3WM;->A02()V

    :cond_0
    return-void
.end method

.method public A4Q()I
    .locals 1

    const/16 v0, 0x80

    return v0
.end method

.method public AN5(B)V
    .locals 4

    iget-object v2, p0, LX/3WM;->A0C:[B

    iget v0, p0, LX/3WM;->A01:I

    add-int/lit8 v1, v0, 0x1

    iput v1, p0, LX/3WM;->A01:I

    aput-byte p1, v2, v0

    array-length v0, v2

    if-ne v1, v0, :cond_0

    const/4 v0, 0x0

    invoke-virtual {p0, v2, v0}, LX/3WM;->A05([BI)V

    iput v0, p0, LX/3WM;->A01:I

    :cond_0
    iget-wide v2, p0, LX/3WM;->A0A:J

    const-wide/16 v0, 0x1

    add-long/2addr v2, v0

    iput-wide v2, p0, LX/3WM;->A0A:J

    return-void
.end method

.method public reset()V
    .locals 6

    const-wide/16 v2, 0x0

    iput-wide v2, p0, LX/3WM;->A0A:J

    iput-wide v2, p0, LX/3WM;->A0B:J

    const/4 v5, 0x0

    iput v5, p0, LX/3WM;->A01:I

    const/4 v4, 0x0

    :goto_0
    iget-object v1, p0, LX/3WM;->A0C:[B

    array-length v0, v1

    if-ge v4, v0, :cond_0

    aput-byte v5, v1, v4

    add-int/lit8 v4, v4, 0x1

    goto :goto_0

    :cond_0
    iput v5, p0, LX/3WM;->A00:I

    :goto_1
    iget-object v1, p0, LX/3WM;->A0D:[J

    array-length v0, v1

    if-eq v5, v0, :cond_1

    aput-wide v2, v1, v5

    add-int/lit8 v5, v5, 0x1

    goto :goto_1

    :cond_1
    return-void
.end method

.method public update([BII)V
    .locals 4

    :goto_0
    iget v0, p0, LX/3WM;->A01:I

    if-eqz v0, :cond_0

    if-lez p3, :cond_0

    aget-byte v0, p1, p2

    invoke-virtual {p0, v0}, LX/3WM;->AN5(B)V

    add-int/lit8 p2, p2, 0x1

    add-int/lit8 p3, p3, -0x1

    goto :goto_0

    :cond_0
    :goto_1
    iget-object v0, p0, LX/3WM;->A0C:[B

    array-length v0, v0

    if-le p3, v0, :cond_1

    invoke-virtual {p0, p1, p2}, LX/3WM;->A05([BI)V

    add-int/2addr p2, v0

    sub-int/2addr p3, v0

    iget-wide v2, p0, LX/3WM;->A0A:J

    int-to-long v0, v0

    add-long/2addr v2, v0

    iput-wide v2, p0, LX/3WM;->A0A:J

    goto :goto_1

    :cond_1
    :goto_2
    if-lez p3, :cond_2

    aget-byte v0, p1, p2

    invoke-virtual {p0, v0}, LX/3WM;->AN5(B)V

    add-int/lit8 p2, p2, 0x1

    add-int/lit8 p3, p3, -0x1

    goto :goto_2

    :cond_2
    return-void
.end method
